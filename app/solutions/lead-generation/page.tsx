import Image from "next/image"
import { ArrowUpRight, Check, Mail, Plus, ChevronRight, Search, Target } from "lucide-react"

export default function RealTimeLeadGeneration() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white text-gray-900 p-4 md:p-8 lg:p-12">
      <main className="w-full max-w-6xl mx-auto grid gap-12 py-12">
        {/* Hero Section */}
        <section className="grid md:grid-cols-2 gap-8 items-center">
          <div className="space-y-6">
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-headline font-semibold leading-tight">
              Real-Time Lead Generation for Recruiters
            </h1>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-headline font-regular text-[#0075DE] leading-tight">
              Get fresh job leads without lifting a finger
            </h2>
            <p className="text-lg text-gray-700 max-w-md">
              JOTIQ scans top job boards 24/7 and delivers new leads directly to your dashboard. Act on high-quality job opportunities as soon as they go live.
            </p>
          </div>
          <div className="relative flex justify-center items-center">
          <Image
            // src="/solutions/lead-genration.png"
            alt="Real-Time Lead Generation Dashboard Illustration"
            width={400}
            height={300}
            className="rounded-3xl shadow-2xl border border-gray-100 object-cover"
          />
        </div>
        </section>

        {/* What You Can Do / Why It Works Sections */}
        <section className="grid md:grid-cols-2 gap-12 mt-12">
          <div>
            <h3 className="text-2xl font-headline font-bold mb-4">What You Can Do</h3>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <Search className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Monitor live job listings across major job boards</p>
              </li>
              <li className="flex items-start gap-3">
                <Target className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Filter and save relevant opportunities</p>
              </li>
              <li className="flex items-start gap-3">
                <ArrowUpRight className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Track all leads from one place</p>
              </li>
            </ul>
          </div>
          <div>
            <h3 className="text-2xl font-headline font-bold mb-4">Why It Works</h3>
            <p className="text-lg text-gray-700 mb-6">
              Job boards are constantly changing. Manually checking each one takes time and leads to missed opportunities. JOTIQ delivers the newest job leads instantly so you never fall behind.
            </p>
          </div>
        </section>

        {/* How It Works Section */}
        <section className="mt-12">
          <h3 className="text-2xl font-headline font-bold mb-6 text-center">How It Works (Step-by-Step)</h3>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center space-y-4">
              <div className="w-16 h-16 flex items-center justify-center bg-[#0075DE] text-white rounded-full text-2xl font-bold mx-auto">
                1
              </div>
              <h4 className="text-xl font-semibold">JOTIQ scans job boards in real time</h4>
            </div>
            <div className="text-center space-y-4">
              <div className="w-16 h-16 flex items-center justify-center bg-[#0075DE] text-white rounded-full text-2xl font-bold mx-auto">
                2
              </div>
              <h4 className="text-xl font-semibold">New leads appear in your dashboard</h4>
            </div>
            <div className="text-center space-y-4">
              <div className="w-16 h-16 flex items-center justify-center bg-[#0075DE] text-white rounded-full text-2xl font-bold mx-auto">
                3
              </div>
              <h4 className="text-xl font-semibold">You filter and act on hot jobs</h4>
            </div>
          </div>
        </section>

        {/* Key Benefits & Testimonial */}
        <section className="grid md:grid-cols-2 gap-12 mt-12">
          <div>
            <h3 className="text-2xl font-headline font-bold mb-4">Key Benefits</h3>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <Check className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Save hours every week on lead sourcing</p>
              </li>
              <li className="flex items-start gap-3">
                <Check className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Centralize job leads in one smart dashboard</p>
              </li>
              <li className="flex items-start gap-3">
                <Check className="w-6 h-6 text-[#0075DE] shrink-0 mt-1" />
                <p className="text-lg text-gray-700">Capture opportunities before your competitors do</p>
              </li>
            </ul>
          </div>
          <div className="flex items-center justify-center">
            <p className="text-2xl font-semibold italic text-gray-800 text-center">
              &quot;Start sourcing better job leads in minutes&quot;
            </p>
          </div>
        </section>

        {/* Call to Action */}
        <section className="flex flex-col items-center justify-center mt-12 space-y-6">
          <button className="bg-[#0075DE] hover:bg-blue-700 text-white px-8 py-4 text-xl font-semibold rounded-lg shadow-lg transition-colors duration-200">
            Start Scraping
          </button>
          <p className="text-md text-gray-600 text-center max-w-lg">
            Want to follow up faster? Pair this with {' '}
            <a 
              href="/solutions/automated-outreach"
              className="text-[#0075DE] hover:text-blue-700 transition-colors duration-200"
            >
            Automated Outreach 
            </a>
            {' '} or {' '}
            <a 
              href="/solutions/smart-data-upload-extraction"
              className="text-[#0075DE] hover:text-blue-700 transition-colors duration-200"
            >
            Smart Data Upload
            </a>
          </p>
        </section>
      </main>
    </div>
  )
}